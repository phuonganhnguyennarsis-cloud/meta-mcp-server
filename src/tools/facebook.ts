...
To post the same photo to several Pages in one go (e.g. all Narsis Pages), pass page_id as an array of Page IDs, or "all" to target every Page configured in META_PAGE_ID.`,
  ...
  // --- meta_create_page_story ------------------------------------------------
  const CreatePageStorySchema = z
    .object({
      page_id: pageIdField,
      media_type: z.enum(["IMAGE", "VIDEO"]).describe("'IMAGE' for a photo Story, 'VIDEO' for a video Story."),
      media_url: z.string().url().describe("Publicly reachable URL of the image (jpg) or video (mp4/mov) to post as a Story."),
    })
    .strict();
  ...
  server.registerTool(
    "meta_create_page_story",
    {
      title: "Create Facebook Page Story",
      description: `Publish a single image or video to a Facebook Page's Story (visible 24 hours). Two-step Meta workflow (upload the media unpublished, then attach it to a Story) handled as one call.
...
Returns JSON: { story_id, media_id } for a single Page, or { results: [{ page_id, story_id?, media_id?, error? }], succeeded, failed } when posting to multiple Pages at once.

IMPORTANT LIMITATION: like Instagram Stories, Facebook Page Stories published via the Graph API are plain image/video only — no music stickers, link/swipe-up stickers, polls, or other interactive stickers. Those can only be added by hand inside the Facebook app.

Requires 'pages_manage_posts' permission for each Page. The media_url must be publicly accessible (no auth, no localhost).

Use when: "Post this photo/video to our Facebook Page Story."
To post the same Story to several Pages in one go (e.g. all Narsis Pages), pass page_id as an array of Page IDs, or "all" to target every Page configured in META_PAGE_ID.`,
      inputSchema: CreatePageStorySchema.shape,
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    },
    async (params: z.infer<typeof CreatePageStorySchema>) => {
      try {
        const pageIds = resolvePageIds(params.page_id);
        const postOne = async (pageId: string) => {
          const pageToken = await getPageAccessToken(pageId);
          if (params.media_type === "VIDEO") {
            const video: any = await graphPost(
              `${pageId}/videos`,
              { file_url: params.media_url, published: false },
              pageToken
            );
            const story: any = await graphPost(`${pageId}/video_stories`, { video_id: video.id }, pageToken);
            return { page_id: pageId, story_id: story.post_id ?? story.id, media_id: video.id };
          }
          const photo: any = await graphPost(
            `${pageId}/photos`,
            { url: params.media_url, published: false },
            pageToken
          );
          const story: any = await graphPost(`${pageId}/photo_stories`, { photo_id: photo.id }, pageToken);
          return { page_id: pageId, story_id: story.post_id ?? story.id, media_id: photo.id };
        };
        const results = await Promise.allSettled(pageIds.map(postOne));
        ...
        if (pageIds.length === 1) {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Facebook Page Story published (plain story — no music/link sticker; add those by hand in-app if needed). Story ID: ${output.story_id}`,
              },
            ],
            structuredContent: output,
          };
        ...
        return {
          content: [
            {
              type: "text",
              text: `Story posted to ${succeeded}/${perPage.length} Page(s).${failed > 0 ? ` ${failed} failed — see results for details.` : ""}`,
            },
          ],
          structuredContent: output,
        };
